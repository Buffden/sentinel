...
- Redis keys and channels:
  ...
  - `proximity-episode:{pair_key}` - proximity episode state; hash with fields `episode_start_ms`, `last_seen_ms`; TTL = `PROXIMITY_EPISODE_GAP_MS` (refreshed on each within-threshold ping); canonical pair = `min(a,b):max(a,b)`; written by correlation worker on first detection in a new episode; subsequent pings update `last_seen_ms` and refresh TTL instead of publishing a new candidate
  ...
  - `deviation-state:{entity_id}` - hash; fields: `count`, `episode_start_ms`, `last_processed_ms`, `alert_emitted`; written by alert evaluator; safety TTL = `2 × SIGNAL_LOSS_THRESHOLD_MS`; DEL on `IN_RANGE` event; `last_processed_ms` is the replay guard — ignore events with `timestamp_ms <= last_processed_ms`
  ...
  - `alert-evaluator:leader` - leader election lease key; SET NX PX; compare-and-renew (`SET XX PX`) on heartbeat; compare-before-DEL on release
