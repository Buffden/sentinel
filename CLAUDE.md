...
Before writing or changing any code, Claude must explain the following in plain language a non-expert can follow. Do not skip or abbreviate any point. Do not open an editor until the teach-back is complete and the developer has confirmed understanding.

1. **Concept** — what is the mechanism being built, and why does Sentinel need it at this stage rather than later?
2. **Sentinel intent and ownership** — which correctness property, failure mode, or operational guarantee does this advance? Name it explicitly (idempotency, monotonicity, leader exclusivity, etc.). State which service owns the work and which Kafka topics, Redis keys, DB tables, or Neo4j nodes it reads and writes.
3. **Data flow** — trace the event from its source (telemetry ingestion, Kafka message, timer tick, HTTP request) through every transformation to its final durable or ephemeral destination. Name every hop.
4. **Manual verification** — list the exact commands (redis-cli, kafkacat, psql, curl, logs) the developer will run to confirm it is working and what a passing result looks like. Include at least one failure-boundary check, not only the happy path.

Only after covering all four points: propose code changes.
