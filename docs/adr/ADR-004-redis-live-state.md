...
**Simple key schema.** `entity:live:{entity_id}` is a flat key pointing to a hash of position fields. No complex data structures required  - straightforward GET/HGETALL on read, HSET on write.

**TTL for ghost cleanup.** Entities that stop broadcasting can have their Redis key expired automatically via TTL, preventing stale state from persisting indefinitely on the dashboard.
