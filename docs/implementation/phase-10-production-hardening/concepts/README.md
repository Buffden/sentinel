...
| [provider-failover/](provider-failover/) | CP3e-CP3f | Real Redis and Redpanda verify adsb.fi → `none` → OpenSky failover, five-minute-gated OpenSky → adsb.fi HANDOVER, conservative restart, Kafka-failure behavior, and one intentional publication at a time without spending live OpenSky credits |
