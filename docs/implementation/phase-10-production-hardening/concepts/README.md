...
| [observed-silence/](observed-silence/) | CP3c | The Alert Evaluator counts only the owning provider's coverage as silence: a 7.8 min `kill -9` coordinator outage raised no alerts, recovery credited none of the gap, and an evaluator restart with 44 aircraft past the threshold on wall-clock time raised none |
| [provider-health/](provider-health/) | CP3d | adsb.fi and OpenSky health move through `HEALTHY`, `DEGRADED`, `UNAVAILABLE` and `RECOVERING` in Redis: a deadline timer made adsb.fi `UNAVAILABLE` on time with no request finishing, a broker outage left health `HEALTHY` while coverage closed, and authority stayed adsb.fi through every change |
| [provider-failover/](provider-failover/) | CP3e | Real Redis and Redpanda verify adsb.fi → `none` → OpenSky failover, separate COMMIT/CREDIT revisions, no proactive failback, Kafka-failure conservatism, restart restoration, and one intentional publication at a time without spending live OpenSky credits |
