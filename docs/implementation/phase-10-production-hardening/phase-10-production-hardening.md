...
| CP3e | Failover to OpenSky | Cutting adsb.fi off makes OpenSky authoritative only after a successful OpenSky active cycle, and the switch is logged. A failed publish during the switch leaves authority at `none`. A restart restores a stored `none` or `opensky`, and relinquishes a stored authority whose provider restores `UNAVAILABLE` | Pending |
