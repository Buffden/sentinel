...
| CP7: Dashboard | Live map; alert panel; login page; demo mode (POST /auth/demo, role claim, IP rate limit, WS expiry) | Done — CP7a–m all verified; see [`concepts/dashboard/README.md`](concepts/dashboard/README.md#checkpoint-order) for checkpoint-by-checkpoint status |
