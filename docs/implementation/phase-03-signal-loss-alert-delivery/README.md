...
| CP7: Dashboard | Live map; alert panel; login page; demo mode (POST /auth/demo, role claim, IP rate limit, WS expiry) | In progress — CP7f done (live position WebSocket; see [`concepts/dashboard/README.md`](concepts/dashboard/README.md)). CP7g done — aviation filters as a floating panel, not the generic layer-toggle overlay; see [`concepts/dashboard/filters.md`](concepts/dashboard/filters.md) for the deviation reasoning, the `applyPositionUpdate` merge fix, and two drag-bounds bugs found and fixed. |
