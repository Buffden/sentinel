...
| CP7: Dashboard | Live map; alert panel; login page; demo mode (POST /auth/demo, role claim, IP rate limit, WS expiry) | In progress — CP7f done (live position WebSocket; see [`concepts/dashboard/README.md`](concepts/dashboard/README.md)). CP7g done — aviation filters as a floating panel; see [`concepts/dashboard/filters.md`](concepts/dashboard/filters.md). CP7h done — REST alert hydration (`entities/alert/`, mirrors CP7e's tracked-entity pattern). CP7i done — live alert WebSocket feed via a shared, reference-counted connection singleton; see [`concepts/dashboard/shared-websocket.md`](concepts/dashboard/shared-websocket.md). CP7j done — Vitest added to the dashboard, position staleness and alert dedup proven with tests; see [`concepts/dashboard/frontend-testing.md`](concepts/dashboard/frontend-testing.md). CP7k done — reconnect re-runs REST hydration and re-subscribes, recovering data missed during a drop; proven against a real killed-API disconnect; see [`concepts/dashboard/reconnect-reconciliation.md`](concepts/dashboard/reconnect-reconciliation.md). CP7l done — no code needed, existing CP7f/CP7i wiring already correct; verified with a short-lived crafted JWT rather than a real 3-minute wait; see [`concepts/dashboard/demo-expiry-verification.md`](concepts/dashboard/demo-expiry-verification.md). Only CP7m (full E2E demonstration) remains. |
