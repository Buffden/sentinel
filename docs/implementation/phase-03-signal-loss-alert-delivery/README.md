...
| CP7: Dashboard | Live map; alert panel; login page; demo mode (POST /auth/demo, role claim, IP rate limit, WS expiry) | In progress — CP7f done (websocketClient.ts factory; useLiveFeed hook with 5 s reconnect and demo-expiry handling; MapWidget entity state changed from array to Map; subscribe(bbox) called after REST hydration; applyPositionUpdate monotonicity guard; onDemoExpired threaded via Workspace params; aircraft markers move live without page refresh). CP7g done — deviates from this doc's original "layer-toggle overlay" framing: aviation is still the only map layer, so the generic searchable/collapsible layer-enable overlay is deferred until a second layer domain exists (see dashboard concepts README). Instead, a floating, draggable, collapsible `FilterPanel` (`features/entity-filtering/`) lives inside the Map widget's canvas: callsign search (dims non-matching aircraft, doesn't hide) and an airborne/grounded status dropdown (hard filter, excludes from render data), plus a clear button and a "N of M shown" count. Entity subtype checkboxes and the altitude range slider from the original spec were deliberately deferred — not requested for this pass. Also fixed a real bug the status filter would otherwise have exposed: `applyPositionUpdate` fully overwrote entities on every WebSocket tick, and the WS position-updates frame never carried `on_ground`/`entity_subtype` — together this silently wiped a REST-hydrated entity's ground status the instant its first live position update arrived. Changed to a merge (`TrackedEntityUpdate` — only present keys overwrite) so fields a given source doesn't carry are preserved rather than erased. Residual gap: an entity discovered only via WebSocket (never REST-hydrated) still has no `on_ground` until next hydration/reconnect — the WS payload itself doesn't carry it; fixing that needs a small position-consumer change, not attempted here.

Two bugs found via manual browser testing after initial implementation, both fixed same-session: (1) the drag handler had no bounds check at all, so the panel could be dragged fully outside the map widget and vanish off-screen with no way back short of reload — fixed by clamping to the container's measured dimensions during drag. (2) Collapsing/expanding changes the panel's height but position wasn't re-clamped against the new size — drag the collapsed (small) panel near the bottom edge, then expand: the larger body pushed past the widget boundary. Fixed by re-measuring and clamping inside the collapse-toggle's click handler using `flushSync` (forces the `collapsed` state's DOM update to commit before measuring `panelRef`), not a `useLayoutEffect` — calling `setState` synchronously inside an effect trips this repo's `react-hooks/set-state-in-effect` lint rule as a hard error regardless of a runtime bailout guard, since the linter can't see that the update sometimes no-ops. Lesson for any future floating/draggable UI in this codebase: bounds-check position on every dimension change (drag *and* content-size change), not just drag. |
