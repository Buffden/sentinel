...
| CP7: Dashboard | Live map; alert panel; login page; demo mode (POST /auth/demo, role claim, IP rate limit, WS expiry) | In progress — CP7f done (live position WebSocket; see [`concepts/dashboard/README.md`](concepts/dashboard/README.md)). CP7g done — aviation filters as a floating panel; see [`concepts/dashboard/filters.md`](concepts/dashboard/filters.md). CP7h done — REST alert hydration (`entities/alert/`, mirrors CP7e's tracked-entity pattern). CP7i done — live alert WebSocket feed via a shared, reference-counted connection singleton; see [`concepts/dashboard/cp7i-shared-websocket.md`](concepts/dashboard/cp7i-shared-websocket.md). CP7j done — Vitest added to the dashboard, position staleness and alert dedup proven with tests (verified non-vacuous by breaking each guard and confirming the test fails); see [`concepts/dashboard/cp7j-frontend-testing.md`](concepts/dashboard/cp7j-frontend-testing.md). |
