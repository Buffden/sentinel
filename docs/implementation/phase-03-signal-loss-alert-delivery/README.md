...
| CP7: Dashboard | Live map; alert panel; login page; demo mode (POST /auth/demo, role claim, IP rate limit, WS expiry) | In progress — CP7d done (landing page with Google OAuth + demo login; /dashboard auth guard with 401 redirect; role claim in JWT; WS 4401 demo expiry timer; DemoExpiredBanner; shared TopNav/Footer; NavUserIcons with name tooltip and logout; GET /auth/me; POST /auth/logout) |
