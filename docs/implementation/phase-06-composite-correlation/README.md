...
| CP5A | — | Wire CP2 → CP3B → CP3C → CP4 into `handleProximityCandidate`; publish `COMPOSITE`/`UNSCHEDULED_PROXIMITY`; decision cleanup ordered after the candidate-consumer session's own `commitOffsets()` succeeds (`handleProximityCandidate` itself does not own the commit). Alert Evaluator only | Pending |
| **Pre-CP5B** | — | **Design-only:** resolve out-of-order convergence between `SIGNAL_LOSS` and `COMPOSITE` arriving at the API in either order — no code | **Pending** |
| CP5B | — | API atomic `COMPOSITE` insert + convergent supersession of referenced active alerts, per Pre-CP5B's resolution, in one DB transaction. API only | Pending |
...
CP5A's decision-cleanup ordering (`create decision -> publish output -> finalize claim -> commit input offset -> delete decision`) is already the accepted design in `DATA_MODEL.md`'s "Composite claim and decision protocol" (Pre-CP3A) — CP5A's job is to implement it against the real `autoCommit: false` / `commitOffsets()` boundary. That boundary belongs to the candidate-consumer session (`startCandidateConsumerSession`'s `eachMessage` wrapper), not to `handleProximityCandidate` itself, which the session calls and then separately commits after — CP5A's cleanup logic has to be wired at the session boundary, not inside the handler, to actually run after a successful commit. Its required failure tests must include both crash-before-commit (decision survives, redelivery reproduces the same result) and crash-after-commit-before-delete (orphaned decision — a harmless cleanup leak, not a correctness failure).
