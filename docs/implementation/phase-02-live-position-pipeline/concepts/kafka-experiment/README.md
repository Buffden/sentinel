...
This experiment is about getting your hands on Kafka before building anything real. You produce one event, consume it, watch the offsets move, crash and replay — all with `rpk` open in another terminal so you can see what the broker actually thinks is happening.
...
| File | Shows |
| --- | --- |
| [sequence diagram](kafka-experiment-sequence.puml) | Time-ordered interaction: consumer startup, producer run, delivery, autoCommit, crash replay |
