...
Lease release safety is critical: when the leader shuts down, it must compare ownership before deleting the lease key (compare-before-DEL Lua script: `if GET == instance_id then DEL`) so it cannot accidentally delete a lease a new leader has already acquired. Lease renewal is ownership-safe using Lua compare-and-expire: `if GET == instance_id then PEXPIRE` — `SET XX PX` alone is NOT sufficient for renewal because it overwrites the key value without checking the current holder.
