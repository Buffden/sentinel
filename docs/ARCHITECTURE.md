...
| Reads from Redis | `geo-cell:{h3_cell_id}` sorted sets — `ZRANGEBYSCORE` for fresh members only (score >= now - LIVE_PROXIMITY_MAX_AGE_MS) in same-cell + computed k-ring cells |
...
- Proximity candidates are scoped using H3: `ZRANGEBYSCORE geo-cell:{cell}` for the incoming entity's cell + computed k-ring cells, score-filtered to fresh members only (score >= now - `LIVE_PROXIMITY_MAX_AGE_MS`). After fetching positions from `entity:live:*`, rechecks `last_seen_ms` and skips stale candidates. Comparison is O(cell density) rather than O(total entities).
...
- **k-ring radius:** computed from `PROXIMITY_THRESHOLD_METRES` and the H3 cell edge length at `LIVE_H3_RESOLUTION` — not hardcoded. The required radius and resolution values are tuned during implementation based on entity density and the configured proximity threshold.
- **H3 resolutions:** `LIVE_H3_RESOLUTION` (used for Redis `geo-cell:*` sorted sets) and `HISTORY_H3_RESOLUTION` (used for TimescaleDB `geo_cell` column) are separate configuration values. The Correlation Worker uses `LIVE_H3_RESOLUTION` for geo-cell lookups. They may differ; tune during implementation based on entity density and proximity threshold.
- **Episode model:** on first detection of a proximity pair within threshold: (1) Neo4j MERGE edge (idempotency key: `{pair_key}:{episode_start_ms}`); (2) create `proximity-episode:{pair_key}` hash with `candidate_published=0`; (3) publish ONE `proximity.candidates` event to Kafka; (4) set `candidate_published=1`. On subsequent pings within the same episode: check `candidate_published` first — if `0` (Neo4j written but Kafka publish failed in a prior attempt), retry the Kafka publish; if `1`, update `last_seen_ms` with the current event's `timestamp_ms` and refresh TTL; do NOT publish a new event. If `candidate_published` field is absent (known-associate episode — this field is never set for known-associate pairs): update `last_seen_ms` + refresh TTL; never publish to `proximity.candidates`.
...
- **Proximity (supersession model):** inputs arrive via `proximity.candidates` (one per new episode). SIGNAL_LOSS is already emitted immediately when detected — never held back. When proximity arrives, check `alert-state:{entity_id}` (entity still dark) OR `recent-loss:{entity_id}` (entity was dark, has since resumed) for both entities. **Active-dark path:** if `alert-state` key exists with `composite_issued==0`, emit COMPOSITE with `supersedes_alert_ids`, then `HSET alert-state:{entity_id} composite_issued 1` — the flag prevents a second COMPOSITE if further proximity arrives for the same signal-loss episode. **Recent-loss path:** if `recent-loss` key exists (TTL = `COMPOSITE_CORRELATION_WINDOW_MS` enforces the window), emit COMPOSITE, then `DEL recent-loss:{entity_id}` — consuming the key ensures only one COMPOSITE per signal-loss episode. If neither key exists: emit UNSCHEDULED_PROXIMITY.
